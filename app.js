...
const SYMPTOM_OPTIONS = [...SYMPTOMS, "Other"];
const THEME_STORAGE_KEY = "mediband-theme";
const DEMO_PATIENTS = [
  {
    key: "demo-10492",
    patientId: "10492",
  ...
  },
  {
    ...
    spo2: 94,
    status: "Urgent",
    riskLabel: "Serious Risk",
    symptoms: ["Dizziness"],
    updatedAt: "2026-05-02T15:07:00.000Z"
  },
  {
    key: "demo-31776",
    patientId: "31776",
    ...
    spo2: 98,
    status: "Observe",
    riskLabel: "Monitoring Required",
    symptoms: ["Fever"],
    updatedAt: "2026-05-02T15:10:00.000Z"
  },
  {
    key: "demo-44103",
    patientId: "44103",
    ...
    riskLabel: "Low Risk",
    symptoms: [],
    updatedAt: "2026-05-02T15:12:00.000Z"
  }
];
const DEMO_SCAN_PROFILES = Object.fromEntries(DEMO_PATIENTS.map((patient) => [patient.patientId, patient]));
...
function initKiosk() {
  ...
  const rfidInput = byId("rfidInput");
  const demoScanButton = byId("demoScanButton");
  const lookupRfidButton = byId("lookupRfidButton");
  const nameInput = byId("nameInput");
  const bpmInput = byId("bpmInput");
  const spo2Input = byId("spo2Input");
  const otherWrap = byId("otherSymptomWrap");
  const otherInput = byId("otherSymptomInput");

  symptomContainer.innerHTML = SYMPTOM_OPTIONS.map(
    (symptom) => `
  ...
  symptomContainer.querySelectorAll('input[name="symptoms"]').forEach((checkbox) => {
    checkbox.addEventListener("change", () => {
      const otherSelected = form.querySelector('input[value="Other"]').checked;
  ...
  demoScanButton.addEventListener("click", () => {
    const profiles = Object.values(DEMO_SCAN_PROFILES);
    const profile = profiles[Math.floor(Math.random() * profiles.length)];
    const bpm = Math.max(68, Math.min(135, profile.bpm + (Math.floor(Math.random() * 17) - 8)));
    const spo2 = Math.max(88, Math.min(100, profile.spo2 + (Math.floor(Math.random() * 5) - 2)));

    rfidInput.value = profile.patientId;
    nameInput.value = profile.name;
    bpmInput.value = String(bpm);
    spo2Input.value = String(spo2);
    resultMessage.textContent = `Demo scan complete: ${profile.name} wristband loaded with live vitals.`;
  });

  lookupRfidButton.addEventListener("click", () => {
    const profile = DEMO_SCAN_PROFILES[String(rfidInput.value || "").trim()];
    if (!profile) {
      resultMessage.textContent = "RFID not found in the demo wristband registry. Try 10492, 20418, 31776, or 44103.";
      nameInput.value = "";
      bpmInput.value = "";
      spo2Input.value = "";
      return;
    ...
    nameInput.value = profile.name;
    bpmInput.value = String(profile.bpm);
    spo2Input.value = String(profile.spo2);
    resultMessage.textContent = `Wristband ${profile.patientId} loaded for ${profile.name}. Symptoms can now be submitted.`;
  });
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const otherSymptom = String(formData.get("otherSymptom") || "").trim();
    const normalizedSymptoms = symptoms
      .filter((symptom) => symptom !== "Other")
      .concat(otherSymptom ? [otherSymptom] : []);
    const bpm = Number(formData.get("bpm") || 0);
    const spo2 = Number(formData.get("spo2") || 0);
    const status = calculateTriage({ bpm, symptoms: normalizedSymptoms });
    const patientRecord = {
      name,
      patientId,
      bpm,
...
function initReception() {
  ...
  const seedButton = byId("seedDemoButton");
  const randomizeButton = byId("randomizeDemoButton");
  const totals = {
  ...
  if (seedButton) {
    seedButton.addEventListener("click", async () => {
      ...
      seedButton.textContent = "Demo Patients Loaded";
    });
  }

  if (randomizeButton) {
    randomizeButton.addEventListener("click", async () => {
      await randomizeDemoPatients();
    });
  }
...
function updateThemeButton(button) {
  button.textContent = document.body.dataset.theme === "light" ? "Toggle Dark Mode" : "Toggle Light Mode";
}
...
async function seedDemoPatients() {
  const writes = DEMO_PATIENTS.map((patient) => {
    const record = {
    ...
    return set(ref(database, `${PATIENTS_PATH}/${patient.key}`), record);
  });

  await Promise.all(writes);
}

async function randomizeDemoPatients() {
  const randomized = DEMO_PATIENTS.map((patient) => {
    const bpmShift = Math.floor(Math.random() * 25) - 8;
    const spo2Shift = Math.floor(Math.random() * 5) - 2;
    const bpm = Math.max(68, Math.min(135, patient.bpm + bpmShift));
    const spo2 = Math.max(86, Math.min(100, patient.spo2 + spo2Shift));
    const symptomPool = [
      [],
      ["Fever"],
      ["Dizziness"],
    ...
    const symptoms = symptomPool[Math.floor(Math.random() * symptomPool.length)];
    ...
    return { key: patient.key, record };
  });

  await Promise.all(
    randomized.map((patient) => set(ref(database, `${PATIENTS_PATH}/${patient.key}`), patient.record))
  );
}
